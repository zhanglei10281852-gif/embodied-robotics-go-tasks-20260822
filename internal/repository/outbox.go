...
func (r *Repositories) FinishOutbox(ctx context.Context, id string, errValue error) error {
	...
	_, e := r.DB.SQL.ExecContext(ctx, `UPDATE outbox_jobs SET state=CASE WHEN attempts+1>=8 THEN ? ELSE ? END,attempts=attempts+1,last_error=?,available_at=? WHERE id=?`, domain.OutboxDead, domain.OutboxPending, errValue.Error(), r.Clock.Now().Add(time.Second).UTC().Format(time.RFC3339Nano), id)
	return e
}
