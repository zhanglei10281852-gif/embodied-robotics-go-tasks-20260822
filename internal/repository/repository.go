...
func (r MissionRepository) ClaimNext(ctx context.Context, tenant, owner string) (domain.Mission, error) {
	...
	err := r.Parent.DB.WithTx(ctx, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx, `SELECT id,tenant_id,robot_id,status,priority,policy_version,owner,attempt,created_at,updated_at,version FROM missions WHERE tenant_id=? ORDER BY priority DESC,created_at,id LIMIT 1`, tenant)
		m, e := scanMission(row)
		if e != nil {
			return e
		}
