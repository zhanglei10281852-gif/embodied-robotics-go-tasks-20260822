...
func (r MissionRepository) Transition(ctx context.Context, tenant, id, from, to string, version int64) error {
	res, e := r.Parent.DB.SQL.ExecContext(ctx, `UPDATE missions SET status=?,version=version+1,updated_at=? WHERE tenant_id=? AND id=? AND version=?`, to, now(r.Parent.Clock), tenant, id, version)
	if e != nil {
